...
typedef struct {
    ...
    uint8_t  rsv[0x74]; // 0x2C ~ 0x9F, Reserved
    uint8_t  vendor[0x60]; // 0xA0 ~ 0xFF, Vendor specific registers
    HBA_PORT ports[32]; // 0x100 ~ 0x11FF, Port control registers
} HBA_MEM;
...
typedef struct {
    ...
    uint8_t  featurel;  // Feature register LSB
    ...
    uint8_t  featureh;  // Feature register MSB
    uint8_t  countl;    // Count register LSB
    uint8_t  counth;    // Count register MSB
